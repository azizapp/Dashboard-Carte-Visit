...
// Added missing import for ShieldCheckIcon to fix ReferenceError on line 194
...
const AdminSettingsPage: React.FC<any> = ({
    userRole, theme, setTheme, font, setFont, accentColor, setAccentColor, onClose, onRefresh, isLoading, appSettings
}) => {
        ...
        <div className="bg-[#F8FAFC] dark:bg-slate-900 min-h-screen pb-20">
            ...
            <main className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-8">
                    ...
                    {/* بطاقة التخصيص - تظهر فقط للمانجر */}
                    {isManager ? (
                                ...
                                <div className="p-6 bg-slate-50 dark:bg-slate-700/30 rounded-2xl border border-slate-100 dark:border-slate-700">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-slate-400 leading-relaxed">Cette couleur sera appliquée globalement à tous τα عناصر النظام.</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="bg-blue-50 dark:bg-slate-800 p-8 rounded-2xl border border-blue-100 dark:border-slate-700 flex flex-col items-center justify-center text-center">
                            <ShieldCheckIcon className="w-12 h-12 text-blue-500 mb-4" />
                            <h3 className="font-bold text-slate-800 dark:text-white">Accès Restreint</h3>
                            <p className="text-sm text-slate-500 max-w-xs mt-2">Seul le Manager Général a l'autorisation de modifier l'identité visuelle du système.</p>
                        </div>
                    )}

                    {/* إدارة المستخدمين - تظهر لكل من المانجر والأدمن */}
                    <section className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
