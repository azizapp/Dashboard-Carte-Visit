...
interface DistributorSettingsPageProps {
...
const DistributorSettingsPage: React.FC<DistributorSettingsPageProps> = ({
  authenticatedUser, theme, setTheme, font, setFont, fontSize, setFontSize, fontWeight, setFontWeight,
  accentColor, setAccentColor, mode, setMode, onRefresh, isLoading, onLogout, onClose, onResetSettings
}) => {
  ...
  return (
    <div className="bg-[#F7F8FA] dark:bg-slate-900 min-h-screen">
      ...
      <main className="max-w-xl mx-auto p-4 space-y-8 pb-20 animate-in fade-in duration-500">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden divide-y divide-slate-50 dark:divide-slate-700">
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[14px] font-bold text-slate-800 dark:text-slate-100">Taille du texte</p>
                <span className="text-[11px] font-black text-[var(--accent-color)]">{fontSize}px</span>
              </div>
              <div className="flex bg-slate-50 dark:bg-slate-700/50 p-1 rounded-xl gap-1">
                {fontSizes.map(s => (
                  <button
                    key={s.value}
                    onClick={() => setFontSize(s.value)}
                    className={`flex-1 py-2 text-[11px] font-bold rounded-lg transition-all ${fontSize === s.value ? 'bg-white dark:bg-slate-600 text-[var(--accent-color)] dark:text-white shadow-sm' : 'text-slate-400'}`}
                  >
                    {s.label}
                  </button>
                ))}
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[14px] font-bold text-slate-800 dark:text-slate-100">Épaisseur du texte</p>
                <span className="text-[11px] font-black text-[var(--accent-color)]">{fontWeight}</span>
              </div>
              <div className="flex bg-slate-50 dark:bg-slate-700/50 p-1 rounded-xl gap-1">
                {fontWeights.map(w => (
                  <button
                    ...
                    className={`flex-1 py-2 text-[11px] rounded-lg transition-all ${fontWeight === w.value ? 'bg-white dark:bg-slate-600 text-[var(--accent-color)] dark:text-white shadow-sm font-bold' : 'text-slate-400'}`}
        ...
        <p className="text-center text-[10px] text-slate-400 mt-6 font-medium">Apollo Ayoware • Version 6.1.4</p>
      </main>
